...
import ipl.restapi.enums.UserValidatiEnum;
import ipl.restapi.service.UserValidatService;
...
public class UserValidatController {
    ...
    @ResponseBody
    public String checkData(@RequestParam(value = "checkvalue") String param,
        ...
        if (StringUtils.isBlank(param) || type.toString() == null) {
            result = JacksonUtil.bean2Json(ResultFormat.build("0", "校验值/检验类型不能为空", 1, "check", null));
        }
        ...
        if (type != UserValidatiEnum.USER_NAME.getType() && type != UserValidatiEnum.USER_EMAIL.getType()) {
            result = JacksonUtil.bean2Json(ResultFormat.build("0", "校验值的类型错误,1-username;3-email", 1, "check", null));
        }
        ...
        try {
            ...
            result = JacksonUtil.bean2Json(ResultFormat.build("0", "服务器维护，请联系站长", 1, "check", null));
        }
        return result;
    }

...
import ipl.restapi.service.UserLoginRegistService;
...
public class UserLoginRegistServiceImpl implements UserLoginRegistService {
    ...
    public String userLogin(String email, String password, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (email == null || password == null) {
            ...
            return JacksonUtil.bean2Json(ResultFormat.build("0", "登录失败，必须提供用户邮箱和密码", 1, "login", null));
        }
        ...
        if (list.size() != 1) {
            ...
            return JacksonUtil.bean2Json(ResultFormat.build("0", "登录失败，无此邮箱", 1, "login", null));
        }
        ...
        if (!DigestUtils.md5DigestAsHex(requestPass.getBytes()).equals(user.getPassword())) {
            return JacksonUtil.bean2Json(ResultFormat.build("0", "登录失败，用户密码不正确", 1, "login", null));
        }
...
//        response.addHeader("Set-Cookie", "uid=112; Path=/; HttpOnly");
    ...
    public String createUser(UserInfo user) {
        ...
        if (user.getUsername() == null || user.getPassword() == null || user.getEmail() == null) {
            return JacksonUtil.bean2Json(ResultFormat.build("0", "username,email,password没有完整提供", 1, "register", null));
        }
//        设置注册时间
...
//        设置登录次数
        ...
        try {
            ...
            return JacksonUtil.bean2Json(ResultFormat.build("0", "注册失败，请联系站长", 1, "register", null));
        }
        return JacksonUtil.bean2Json(ResultFormat.build("1", "注册成功", 0, "register", null));
    }
}
